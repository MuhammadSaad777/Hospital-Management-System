	...
	Storage<Bill*>& bills, Storage<Prescription*>& prescriptions, Admin*& admin)
{
	...
	while (window.isOpen())
	{
				...
				if (mouseClick->button == sf::Mouse::Button::Left)
				{
					...
					else if (State == 11)
					{
						...
						else if (bookbutton11.getGlobalBounds().contains(mousePosition))
						{
							...
							else
							{
								...
								// validate specialization — find matching doctors
								Doctor** alldocs2 = doctors.getall();
								int dcount2 = doctors.getsize();
								Doctor* chosenDoc = nullptr;

								for (int d = 0; d < dcount2; d++)
								{
									if (alldocs2[d]->getId() == docid)
									{
										// case insensitive specialization compare
										const char* ds = alldocs2[d]->getspecialization();
										bool match = true;
										int si = 0;
										...
										if (spec[si] != '\0' || ds[si] != '\0') match = false;
										if (match) { chosenDoc = alldocs2[d]; break; }
									}
								}
								...
								else
								{
									...
									else
									{
										...
										Appointment** allapps2 = appointments.getall();
										int acount2 = appointments.getsize();
										for (int a = 0; a < acount2; a++)
										{
											if (allapps2[a]->getdocid() == docid &&
												Appointment::Comparechars(allapps2[a]->getdate(), date) &&
												Appointment::Comparechars(allapps2[a]->getslot(), slot) &&
												!Appointment::Comparechars(allapps2[a]->getstatus(), "cancelled"))
											{
												slottaken = true;
												break;
											}
										...
										else
										{
											...
											*Patientcurrent -= chosenDoc->getfee();
											...
											int newappid = 0;
											for (int a = 0; a < acount2; a++)
												if (allapps2[a]->getid() > newappid)
													newappid = allapps2[a]->getid();
											newappid++;

											// generate new bill ID
											Bill** allbills3 = bills.getall();
											int bcount3 = bills.getsize();
											int newbillid = 0;
											for (int b = 0; b < bcount3; b++)
												if (allbills3[b]->getid() > newbillid)
													newbillid = allbills3[b]->getid();
											newbillid++;

											// get today date for bill
											time_t tnow = time(0);
											tm* tloc = localtime(&tnow);
											char todaystr[11];
											strftime(todaystr, 11, "%d-%m-%Y", tloc);
											...
											// build appointment CSV line
											char appline2[200];
											int ali = 0;

											auto intToStr = [](int v, char* out) 
												{
												if (v == 0) { out[0] = '0'; out[1] = '\0'; return; }
												char tmp[20]; int ti = 0;
												while (v > 0) { tmp[ti++] = '0' + v % 10;v /= 10; }
												int oi = 0;
												for (int x = ti - 1;x >= 0;x--) out[oi++] = tmp[x];
												out[oi] = '\0';
												};

											char s1[10]; intToStr(newappid, s1);
											for (int x = 0;s1[x] != '\0';x++) appline2[ali++] = s1[x];
											appline2[ali++] = ',';

											char s2[10]; intToStr(Patientcurrent->getId(), s2);
											for (int x = 0;s2[x] != '\0';x++) appline2[ali++] = s2[x];
											appline2[ali++] = ',';

											char s3[10]; intToStr(docid, s3);
											for (int x = 0;s3[x] != '\0';x++) appline2[ali++] = s3[x];
											appline2[ali++] = ',';

											for (int x = 0;date[x] != '\0';x++) appline2[ali++] = date[x];
											appline2[ali++] = ',';
											for (int x = 0;slot[x] != '\0';x++) appline2[ali++] = slot[x];
											appline2[ali++] = ',';
											const char* ps = "pending";
											for (int x = 0;ps[x] != '\0';x++) appline2[ali++] = ps[x];
											appline2[ali] = '\0';

											FileHandler::appendrecord("appointments.txt", appline2);
											...
											char billline3[200];
											int bli = 0;
											char bs1[10]; intToStr(newbillid, bs1);
											for (int x = 0;bs1[x] != '\0';x++) billline3[bli++] = bs1[x];
											billline3[bli++] = ',';
											char bs2[10]; intToStr(Patientcurrent->getId(), bs2);
											for (int x = 0;bs2[x] != '\0';x++) billline3[bli++] = bs2[x];
											billline3[bli++] = ',';
											char bs3[10]; intToStr(newappid, bs3);
											for (int x = 0;bs3[x] != '\0';x++) billline3[bli++] = bs3[x];
											billline3[bli++] = ',';
											float fv = chosenDoc->getfee();
											int fint = (int)fv;
											char ftmp[10]; int fti = 0; char fstr[20]; int fsi = 0;
											if (fint == 0) { fstr[fsi++] = '0'; }
											else { while (fint > 0) { ftmp[fti++] = '0' + fint % 10;fint /= 10; }for (int x = fti - 1;x >= 0;x--)fstr[fsi++] = ftmp[x]; }
											fstr[fsi++] = '.'; fstr[fsi++] = '0'; fstr[fsi++] = '0';
											fstr[fsi] = '\0';
											for (int x = 0;fstr[x] != '\0';x++) billline3[bli++] = fstr[x];
											billline3[bli++] = ',';
											const char* us = "unpaid";
											for (int x = 0;us[x] != '\0';x++) billline3[bli++] = us[x];
											billline3[bli++] = ',';
											for (int x = 0;todaystr[x] != '\0';x++) billline3[bli++] = todaystr[x];
											billline3[bli] = '\0';

											FileHandler::appendrecord("bills.txt", billline3);

											// update patient balance in file
											char patline4[300];
											int ni4 = 0;
											char ps1[10]; intToStr(Patientcurrent->getId(), ps1);
											for (int x = 0;ps1[x] != '\0';x++) patline4[ni4++] = ps1[x];
											patline4[ni4++] = ',';
											const char* nm4 = Patientcurrent->getName();
											for (int x = 0;nm4[x] != '\0';x++) patline4[ni4++] = nm4[x];
											patline4[ni4++] = ',';
											int agev4 = Patientcurrent->getage();
											char agetmp4[5];int ati7 = 0;char agestr4[5];int asi7 = 0;
											if (agev4 == 0) { agestr4[asi7++] = '0'; }
											else { while (agev4 > 0) { agetmp4[ati7++] = '0' + agev4 % 10;agev4 /= 10; }for (int x = ati7 - 1;x >= 0;x--)agestr4[asi7++] = agetmp4[x]; }
											agestr4[asi7] = '\0';
											for (int x = 0;agestr4[x] != '\0';x++) patline4[ni4++] = agestr4[x];
											patline4[ni4++] = ',';
											patline4[ni4++] = Patientcurrent->getgender();
											patline4[ni4++] = ',';
											const char* con4 = Patientcurrent->getcontact();
											for (int x = 0;con4[x] != '\0';x++) patline4[ni4++] = con4[x];
											patline4[ni4++] = ',';
											const char* pw4 = Patientcurrent->getPass();
											for (int x = 0;pw4[x] != '\0';x++) patline4[ni4++] = pw4[x];
											patline4[ni4++] = ',';
											float bal4 = Patientcurrent->getbalance();
											int balint4 = (int)bal4;
											char baltmp4[10];int bti5 = 0;char balstr4[20];int bsi5 = 0;
											if (balint4 == 0) { balstr4[bsi5++] = '0'; }
											else { while (balint4 > 0) { baltmp4[bti5++] = '0' + balint4 % 10;balint4 /= 10; }for (int x = bti5 - 1;x >= 0;x--)balstr4[bsi5++] = baltmp4[x]; }
											balstr4[bsi5] = '\0';
											for (int x = 0;balstr4[x] != '\0';x++) patline4[ni4++] = balstr4[x];
											patline4[ni4] = '\0';
					...
					else if (State == 12)
					{
						...
						else if (cancelbutton12.getGlobalBounds().contains(mousePosition))
						{
							...
							Appointment** allapps = appointments.getall();
							int acount = appointments.getsize();

							for (int i = 0; i < acount; i++)
							{
								if (allapps[i]->getid() == appid &&
									allapps[i]->getpatid() == Patientcurrent->getId() &&
									Appointment::Comparechars(allapps[i]->getstatus(), "pending"))
								{
									found = true;
									...
									int dcount = doctors.getsize();
									for (int d = 0; d < dcount; d++)
									{
										if (alldocs[d]->getId() == allapps[i]->getdocid())
										{
											fee = alldocs[d]->getfee();
											break;
										}
									}
									...
									*Patientcurrent += fee;
									...
									allapps[i]->setstatus("cancelled");

									// build appointment CSV line
									char appline[200];
									int ai = 0;
									int av = allapps[i]->getid();
									char atmp[10]; int ati4 = 0; char astr2[10]; int asi4 = 0;
									if (av == 0) { astr2[asi4++] = '0'; }
									else { while (av > 0) { atmp[ati4++] = '0' + av % 10; av /= 10; } for (int x = ati4 - 1; x >= 0; x--) astr2[asi4++] = atmp[x]; }
									astr2[asi4] = '\0';
									for (int x = 0; astr2[x] != '\0'; x++) appline[ai++] = astr2[x];
									appline[ai++] = ',';

									int pv = allapps[i]->getpatid();
									char ptmp2[10]; int pti2 = 0; char pstr2[10]; int psi2 = 0;
									if (pv == 0) { pstr2[psi2++] = '0'; }
									else { while (pv > 0) { ptmp2[pti2++] = '0' + pv % 10; pv /= 10; } for (int x = pti2 - 1; x >= 0; x--) pstr2[psi2++] = ptmp2[x]; }
									pstr2[psi2] = '\0';
									for (int x = 0; pstr2[x] != '\0'; x++) appline[ai++] = pstr2[x];
									appline[ai++] = ',';

									int dv = allapps[i]->getdocid();
									char dtmp[10]; int dti = 0; char dstr[10]; int dsi = 0;
									if (dv == 0) { dstr[dsi++] = '0'; }
									else { while (dv > 0) { dtmp[dti++] = '0' + dv % 10; dv /= 10; } for (int x = dti - 1; x >= 0; x--) dstr[dsi++] = dtmp[x]; }
									dstr[dsi] = '\0';
									for (int x = 0; dstr[x] != '\0'; x++) appline[ai++] = dstr[x];
									appline[ai++] = ',';

									const char* dt2 = allapps[i]->getdate();
									for (int x = 0; dt2[x] != '\0'; x++) appline[ai++] = dt2[x];
									appline[ai++] = ',';

									const char* sl = allapps[i]->getslot();
									for (int x = 0; sl[x] != '\0'; x++) appline[ai++] = sl[x];
									appline[ai++] = ',';

									const char* cancelled = "cancelled";
									for (int x = 0; cancelled[x] != '\0'; x++) appline[ai++] = cancelled[x];
									appline[ai] = '\0';

									FileHandler::modifyrecord("appointments.txt",
										allapps[i]->getid(), appline);

									// update bill status to cancelled
									Bill** allbills2 = bills.getall();
									int bcount2 = bills.getsize();
									for (int b = 0; b < bcount2; b++)
									{
										if (allbills2[b]->getappid() == appid)
										{
											allbills2[b]->setstatus("cancelled");

											char billline2[200];
											int bi2 = 0;
											int biv2 = allbills2[b]->getid();
											char btmp2[10]; int bti3 = 0; char bstr2[10]; int bsi3 = 0;
											if (biv2 == 0) { bstr2[bsi3++] = '0'; }
											else { while (biv2 > 0) { btmp2[bti3++] = '0' + biv2 % 10; biv2 /= 10; } for (int x = bti3 - 1; x >= 0; x--) bstr2[bsi3++] = btmp2[x]; }
											bstr2[bsi3] = '\0';
											for (int x = 0; bstr2[x] != '\0'; x++) billline2[bi2++] = bstr2[x];
											billline2[bi2++] = ',';

											int piv2 = allbills2[b]->getpatid();
											char ptmp3[10]; int pti3 = 0; char pstr3[10]; int psi3 = 0;
											if (piv2 == 0) { pstr3[psi3++] = '0'; }
											else { while (piv2 > 0) { ptmp3[pti3++] = '0' + piv2 % 10; piv2 /= 10; } for (int x = pti3 - 1; x >= 0; x--) pstr3[psi3++] = ptmp3[x]; }
											pstr3[psi3] = '\0';
											for (int x = 0; pstr3[x] != '\0'; x++) billline2[bi2++] = pstr3[x];
											billline2[bi2++] = ',';

											int aiv2 = allbills2[b]->getappid();
											char atmp2[10]; int ati5 = 0; char astr3[10]; int asi5 = 0;
											if (aiv2 == 0) { astr3[asi5++] = '0'; }
											else { while (aiv2 > 0) { atmp2[ati5++] = '0' + aiv2 % 10; aiv2 /= 10; } for (int x = ati5 - 1; x >= 0; x--) astr3[asi5++] = atmp2[x]; }
											astr3[asi5] = '\0';
											for (int x = 0; astr3[x] != '\0'; x++) billline2[bi2++] = astr3[x];
											billline2[bi2++] = ',';

											float amtv2 = allbills2[b]->getamount();
											int amtint2 = (int)amtv2;
											char amttmp2[10]; int amtti2 = 0; char amtstr2[20]; int amtsi2 = 0;
											if (amtint2 == 0) { amtstr2[amtsi2++] = '0'; }
											else { while (amtint2 > 0) { amttmp2[amtti2++] = '0' + amtint2 % 10; amtint2 /= 10; } for (int x = amtti2 - 1; x >= 0; x--) amtstr2[amtsi2++] = amttmp2[x]; }
											amtstr2[amtsi2++] = '.'; amtstr2[amtsi2++] = '0'; amtstr2[amtsi2++] = '0';
											amtstr2[amtsi2] = '\0';
											for (int x = 0; amtstr2[x] != '\0'; x++) billline2[bi2++] = amtstr2[x];
											billline2[bi2++] = ',';

											const char* cs = "cancelled";
											for (int x = 0; cs[x] != '\0'; x++) billline2[bi2++] = cs[x];
											billline2[bi2++] = ',';

											const char* bd = allbills2[b]->getdate();
											for (int x = 0; bd[x] != '\0'; x++) billline2[bi2++] = bd[x];
											billline2[bi2] = '\0';

											FileHandler::modifyrecord("bills.txt",
												allbills2[b]->getid(), billline2);
											break;
										}
									}
									...
									char idtmp3[10]; int iti3 = 0; char idstr3[10]; int isi3 = 0;
									if (idv3 == 0) { idstr3[isi3++] = '0'; }
									else { while (idv3 > 0) { idtmp3[iti3++] = '0' + idv3 % 10; idv3 /= 10; } for (int x = iti3 - 1; x >= 0; x--) idstr3[isi3++] = idtmp3[x]; }
									idstr3[isi3] = '\0';
									for (int x = 0; idstr3[x] != '\0'; x++) patline3[ni3++] = idstr3[x];
									patline3[ni3++] = ',';
									const char* nm3 = Patientcurrent->getName();
									for (int x = 0; nm3[x] != '\0'; x++) patline3[ni3++] = nm3[x];
									patline3[ni3++] = ',';
									int agev3 = Patientcurrent->getage();
									char agetmp3[5]; int ati6 = 0; char agestr3[5]; int asi6 = 0;
									if (agev3 == 0) { agestr3[asi6++] = '0'; }
									else { while (agev3 > 0) { agetmp3[ati6++] = '0' + agev3 % 10; agev3 /= 10; } for (int x = ati6 - 1; x >= 0; x--) agestr3[asi6++] = agetmp3[x]; }
									agestr3[asi6] = '\0';
									for (int x = 0; agestr3[x] != '\0'; x++) patline3[ni3++] = agestr3[x];
									patline3[ni3++] = ',';
									patline3[ni3++] = Patientcurrent->getgender();
									patline3[ni3++] = ',';
									const char* con3 = Patientcurrent->getcontact();
									for (int x = 0; con3[x] != '\0'; x++) patline3[ni3++] = con3[x];
									patline3[ni3++] = ',';
									const char* pw3 = Patientcurrent->getPass();
									for (int x = 0; pw3[x] != '\0'; x++) patline3[ni3++] = pw3[x];
									patline3[ni3++] = ',';
									float bal3 = Patientcurrent->getbalance();
									int balint3 = (int)bal3;
									char baltmp3[10]; int bti4 = 0; char balstr3[20]; int bsi4 = 0;
									if (balint3 == 0) { balstr3[bsi4++] = '0'; }
									else { while (balint3 > 0) { baltmp3[bti4++] = '0' + balint3 % 10; balint3 /= 10; } for (int x = bti4 - 1; x >= 0; x--) balstr3[bsi4++] = baltmp3[x]; }
									balstr3[bsi4] = '\0';
									for (int x = 0; balstr3[x] != '\0'; x++) patline3[ni3++] = balstr3[x];
									patline3[ni3] = '\0';
									FileHandler::modifyrecord("patients.txt",
										Patientcurrent->getId(), patline3);

									// update balance display
									char patbalance3[100] = "BALANCE: PKR ";
									int newval3 = 13;
									for (int x = 0; balstr3[x] != '\0'; x++)
										patbalance3[newval3++] = balstr3[x];
									patbalance3[newval3] = '\0';
									patienttexts.setString(patbalance3);
					...
					else if (State == 16)
					{
						...
						{
							int billid = atoi(paybillbox16.getvalue());
							bool found = false;

							Bill** allbills = bills.getall();
							int bcount = bills.getsize();

							for (int i = 0; i < bcount; i++)
							{
								if (allbills[i]->getid() == billid &&
									allbills[i]->getpatid() == Patientcurrent->getId())
								{
									// check status is unpaid
									...
									// check balance
									if (Patientcurrent->getbalance() < allbills[i]->getamount())
									{
										resulttext16.setFillColor(sf::Color::Red);
									...
									// deduct balance
									*Patientcurrent -= allbills[i]->getamount();

									// update bill status
									allbills[i]->setstatus("paid");
									...
									// bill_id
									int biv = allbills[i]->getid();
									char btmp[10]; int bti = 0;
									char bstr[10]; int bsi = 0;
									if (biv == 0) { bstr[bsi++] = '0'; }
									else { while (biv > 0) { btmp[bti++] = '0' + biv % 10; biv /= 10; } for (int x = bti - 1; x >= 0; x--) bstr[bsi++] = btmp[x]; }
									bstr[bsi] = '\0';
									for (int x = 0; bstr[x] != '\0'; x++) billline[bi++] = bstr[x];
									billline[bi++] = ',';

									// patient_id
									int piv = allbills[i]->getpatid();
									char ptmp[10]; int pti = 0;
									char pstr[10]; int psi = 0;
									if (piv == 0) { pstr[psi++] = '0'; }
									else { while (piv > 0) { ptmp[pti++] = '0' + piv % 10; piv /= 10; } for (int x = pti - 1; x >= 0; x--) pstr[psi++] = ptmp[x]; }
									pstr[psi] = '\0';
									for (int x = 0; pstr[x] != '\0'; x++) billline[bi++] = pstr[x];
									billline[bi++] = ',';

									// appointment_id
									int aiv = allbills[i]->getappid();
									char atmp[10]; int ati2 = 0;
									char astr[10]; int asi2 = 0;
									if (aiv == 0) { astr[asi2++] = '0'; }
									else { while (aiv > 0) { atmp[ati2++] = '0' + aiv % 10; aiv /= 10; } for (int x = ati2 - 1; x >= 0; x--) astr[asi2++] = atmp[x]; }
									astr[asi2] = '\0';
									for (int x = 0; astr[x] != '\0'; x++) billline[bi++] = astr[x];
									billline[bi++] = ',';

									// amount
									float amtv = allbills[i]->getamount();
									int amtint = (int)amtv;
									char amttmp[10]; int amtti = 0;
									char amtstr[20]; int amtsi = 0;
									if (amtint == 0) { amtstr[amtsi++] = '0'; }
									else { while (amtint > 0) { amttmp[amtti++] = '0' + amtint % 10; amtint /= 10; } for (int x = amtti - 1; x >= 0; x--) amtstr[amtsi++] = amttmp[x]; }
									amtstr[amtsi++] = '.'; amtstr[amtsi++] = '0'; amtstr[amtsi++] = '0';
									amtstr[amtsi] = '\0';
									for (int x = 0; amtstr[x] != '\0'; x++) billline[bi++] = amtstr[x];
									billline[bi++] = ',';

									// status
									const char* st = "paid";
									for (int x = 0; st[x] != '\0'; x++) billline[bi++] = st[x];
									billline[bi++] = ',';

									// date
									const char* dt = allbills[i]->getdate();
									for (int x = 0; dt[x] != '\0'; x++) billline[bi++] = dt[x];
									billline[bi] = '\0';

									FileHandler::modifyrecord("bills.txt",
										allbills[i]->getid(), billline);

									// update patient balance in file
									char patline[300];
									int ni = 0;
									int idv2 = Patientcurrent->getId();
									char idtmp2[10]; int iti2 = 0;
									char idstr2[10]; int isi2 = 0;
									if (idv2 == 0) { idstr2[isi2++] = '0'; }
									else { while (idv2 > 0) { idtmp2[iti2++] = '0' + idv2 % 10; idv2 /= 10; } for (int x = iti2 - 1; x >= 0; x--) idstr2[isi2++] = idtmp2[x]; }
									idstr2[isi2] = '\0';
									for (int x = 0; idstr2[x] != '\0'; x++) patline[ni++] = idstr2[x];
									patline[ni++] = ',';
									const char* nm2 = Patientcurrent->getName();
									for (int x = 0; nm2[x] != '\0'; x++) patline[ni++] = nm2[x];
									patline[ni++] = ',';
									int agev2 = Patientcurrent->getage();
									char agetmp2[5]; int ati3 = 0;
									char agestr2[5]; int asi3 = 0;
									if (agev2 == 0) { agestr2[asi3++] = '0'; }
									else { while (agev2 > 0) { agetmp2[ati3++] = '0' + agev2 % 10; agev2 /= 10; } for (int x = ati3 - 1; x >= 0; x--) agestr2[asi3++] = agetmp2[x]; }
									agestr2[asi3] = '\0';
									for (int x = 0; agestr2[x] != '\0'; x++) patline[ni++] = agestr2[x];
									patline[ni++] = ',';
									patline[ni++] = Patientcurrent->getgender();
									patline[ni++] = ',';
									const char* con2 = Patientcurrent->getcontact();
									for (int x = 0; con2[x] != '\0'; x++) patline[ni++] = con2[x];
									patline[ni++] = ',';
									const char* pw2 = Patientcurrent->getPass();
									for (int x = 0; pw2[x] != '\0'; x++) patline[ni++] = pw2[x];
									patline[ni++] = ',';
									float bal2 = Patientcurrent->getbalance();
									int balint2 = (int)bal2;
									char baltmp2[10]; int bti2 = 0;
									char balstr2[20]; int bsi2 = 0;
									if (balint2 == 0) { balstr2[bsi2++] = '0'; }
									else { while (balint2 > 0) { baltmp2[bti2++] = '0' + balint2 % 10; balint2 /= 10; } for (int x = bti2 - 1; x >= 0; x--) balstr2[bsi2++] = baltmp2[x]; }
									balstr2[bsi2] = '\0';
									for (int x = 0; balstr2[x] != '\0'; x++) patline[ni++] = balstr2[x];
									patline[ni] = '\0';
									FileHandler::modifyrecord("patients.txt",
										Patientcurrent->getId(), patline);
						...
						else if (topupbutton17.getGlobalBounds().contains(mousePosition))
						{
							...
							else
							{
								*Patientcurrent += amount;
								...
								char idtmp[10]; int iti = 0;
								char idstr[10]; int isi = 0;
								if (idv == 0) { idstr[isi++] = '0'; }
								else { while (idv > 0) { idtmp[iti++] = '0' + idv % 10; idv /= 10; } for (int x = iti - 1; x >= 0; x--) idstr[isi++] = idtmp[x]; }
								idstr[isi] = '\0';
								for (int x = 0; idstr[x] != '\0'; x++) newline[ni++] = idstr[x];
								newline[ni++] = ',';

								// name
								const char* nm = Patientcurrent->getName();
								for (int x = 0; nm[x] != '\0'; x++) newline[ni++] = nm[x];
								newline[ni++] = ',';

								// age
								int agev = Patientcurrent->getage();
								char agetmp[5]; int ati = 0;
								char agestr[5]; int asi = 0;
								if (agev == 0) { agestr[asi++] = '0'; }
								else { while (agev > 0) { agetmp[ati++] = '0' + agev % 10; agev /= 10; } for (int x = ati - 1; x >= 0; x--) agestr[asi++] = agetmp[x]; }
								agestr[asi] = '\0';
								for (int x = 0; agestr[x] != '\0'; x++) newline[ni++] = agestr[x];
								newline[ni++] = ',';

								// gender
								newline[ni++] = Patientcurrent->getgender();
								newline[ni++] = ',';

								// contact
								const char* con = Patientcurrent->getcontact();
								for (int x = 0; con[x] != '\0'; x++) newline[ni++] = con[x];
								newline[ni++] = ',';

								// password
								const char* pw = Patientcurrent->getPass();
								for (int x = 0; pw[x] != '\0'; x++) newline[ni++] = pw[x];
								newline[ni++] = ',';

								// balance
								float bal = Patientcurrent->getbalance();
								int balint = (int)bal;
								char baltmp[10]; int bti = 0;
								char balstr[20]; int bsi = 0;
								if (balint == 0) { balstr[bsi++] = '0'; }
								else { while (balint > 0) { baltmp[bti++] = '0' + balint % 10; balint /= 10; } for (int x = bti - 1; x >= 0; x--) balstr[bsi++] = baltmp[x]; }
								balstr[bsi] = '\0';
								for (int x = 0; balstr[x] != '\0'; x++) newline[ni++] = balstr[x];
								newline[ni] = '\0';

								FileHandler::modifyrecord("patients.txt",
									Patientcurrent->getId(), newline);
					...
					else if (State == 22)
					{
						...
						else if (markcompletebutton22.getGlobalBounds().contains(mousePosition))
						{
							...
							Appointment** allapps5 = appointments.getall();
							int acount5 = appointments.getsize();

							for (int a = 0; a < acount5; a++)
							{
								if (allapps5[a]->getid() == appid &&
									allapps5[a]->getdocid() == Doctorcurrent->getId() &&
									Appointment::Comparechars(allapps5[a]->getstatus(), "pending") &&
									Appointment::Comparechars(allapps5[a]->getdate(), todaystr2))
								{
									found = true;
									allapps5[a]->setstatus("completed");

									// build appointment CSV line
									char appline3[200];
									int ali3 = 0;

									auto intToStr2 = [](int v, char* out) {
										if (v == 0) { out[0] = '0'; out[1] = '\0'; return; }
										char tmp[20]; int ti = 0;
										while (v > 0) { tmp[ti++] = '0' + v % 10;v /= 10; }
										int oi = 0;
									...
									char s1[10]; intToStr2(allapps5[a]->getid(), s1);
									for (int x = 0;s1[x] != '\0';x++) appline3[ali3++] = s1[x];
									appline3[ali3++] = ',';

									char s2[10]; intToStr2(allapps5[a]->getpatid(), s2);
									for (int x = 0;s2[x] != '\0';x++) appline3[ali3++] = s2[x];
									appline3[ali3++] = ',';

									char s3[10]; intToStr2(allapps5[a]->getdocid(), s3);
									for (int x = 0;s3[x] != '\0';x++) appline3[ali3++] = s3[x];
									appline3[ali3++] = ',';

									const char* dt7 = allapps5[a]->getdate();
									for (int x = 0;dt7[x] != '\0';x++) appline3[ali3++] = dt7[x];
									appline3[ali3++] = ',';

									const char* sl4 = allapps5[a]->getslot();
									for (int x = 0;sl4[x] != '\0';x++) appline3[ali3++] = sl4[x];
									appline3[ali3++] = ',';

									const char* comp = "completed";
									for (int x = 0;comp[x] != '\0';x++) appline3[ali3++] = comp[x];
									appline3[ali3] = '\0';

									FileHandler::modifyrecord("appointments.txt",
										allapps5[a]->getid(), appline3);
					...
					else if (State == 23)
					{
						...
						else if (marknoshbutton23.getGlobalBounds().contains(mousePosition))
						{
							...
							Appointment** allapps6 = appointments.getall();
							int acount6 = appointments.getsize();

							for (int a = 0; a < acount6; a++)
							{
								if (allapps6[a]->getid() == appid2 &&
									allapps6[a]->getdocid() == Doctorcurrent->getId() &&
									Appointment::Comparechars(allapps6[a]->getstatus(), "pending") &&
									Appointment::Comparechars(allapps6[a]->getdate(), todaystr3))
								{
									found2 = true;
									allapps6[a]->setstatus("no-show");

									// build appointment line
									char appline4[200];
									int ali4 = 0;

									auto intToStr3 = [](int v, char* out) {
										if (v == 0) { out[0] = '0'; out[1] = '\0'; return; }
										char tmp[20]; int ti = 0;
										while (v > 0) { tmp[ti++] = '0' + v % 10;v /= 10; }
										int oi = 0;
									...
									char ns1[10]; intToStr3(allapps6[a]->getid(), ns1);
									for (int x = 0;ns1[x] != '\0';x++) appline4[ali4++] = ns1[x];
									appline4[ali4++] = ',';

									char ns2[10]; intToStr3(allapps6[a]->getpatid(), ns2);
									for (int x = 0;ns2[x] != '\0';x++) appline4[ali4++] = ns2[x];
									appline4[ali4++] = ',';

									char ns3[10]; intToStr3(allapps6[a]->getdocid(), ns3);
									for (int x = 0;ns3[x] != '\0';x++) appline4[ali4++] = ns3[x];
									appline4[ali4++] = ',';

									const char* dt8 = allapps6[a]->getdate();
									for (int x = 0;dt8[x] != '\0';x++) appline4[ali4++] = dt8[x];
									appline4[ali4++] = ',';

									const char* sl5 = allapps6[a]->getslot();
									for (int x = 0;sl5[x] != '\0';x++) appline4[ali4++] = sl5[x];
									appline4[ali4++] = ',';

									const char* nsh = "no-show";
									for (int x = 0;nsh[x] != '\0';x++) appline4[ali4++] = nsh[x];
									appline4[ali4] = '\0';

									FileHandler::modifyrecord("appointments.txt",
										allapps6[a]->getid(), appline4);

									// update bill to cancelled
									Bill** allbills6 = bills.getall();
									int bcount6 = bills.getsize();
									for (int b = 0; b < bcount6; b++)
									{
										if (allbills6[b]->getappid() == appid2)
										{
											allbills6[b]->setstatus("cancelled");

											char billline4[200];
											int bli3 = 0;

											char nb1[10]; intToStr3(allbills6[b]->getid(), nb1);
											for (int x = 0;nb1[x] != '\0';x++) billline4[bli3++] = nb1[x];
											billline4[bli3++] = ',';

											char nb2[10]; intToStr3(allbills6[b]->getpatid(), nb2);
											for (int x = 0;nb2[x] != '\0';x++) billline4[bli3++] = nb2[x];
											billline4[bli3++] = ',';

											char nb3[10]; intToStr3(allbills6[b]->getappid(), nb3);
											for (int x = 0;nb3[x] != '\0';x++) billline4[bli3++] = nb3[x];
											billline4[bli3++] = ',';

											float amtv4 = allbills6[b]->getamount();
											int amtint4 = (int)amtv4;
											char amttmp4[10]; int amtti4 = 0;
											char amtstr4[20]; int amtsi4 = 0;
											if (amtint4 == 0) { amtstr4[amtsi4++] = '0'; }
											else {
												while (amtint4 > 0) { amttmp4[amtti4++] = '0' + amtint4 % 10;amtint4 /= 10; }
												for (int x = amtti4 - 1;x >= 0;x--) amtstr4[amtsi4++] = amttmp4[x];
											}
											amtstr4[amtsi4++] = '.';
											amtstr4[amtsi4++] = '0';
											amtstr4[amtsi4++] = '0';
											amtstr4[amtsi4] = '\0';
											for (int x = 0;amtstr4[x] != '\0';x++) billline4[bli3++] = amtstr4[x];
											billline4[bli3++] = ',';

											const char* cs2 = "cancelled";
											for (int x = 0;cs2[x] != '\0';x++) billline4[bli3++] = cs2[x];
											billline4[bli3++] = ',';

											const char* bd2 = allbills6[b]->getdate();
											for (int x = 0;bd2[x] != '\0';x++) billline4[bli3++] = bd2[x];
											billline4[bli3] = '\0';

											FileHandler::modifyrecord("bills.txt",
												allbills6[b]->getid(), billline4);
											break;
										}
									}
					...
					else if (State == 24)
					{
						...
						else if (prescbutton24.getGlobalBounds().contains(mousePosition))
						{
							...
							// validate appointment belongs to doctor and is completed
							bool appvalid = false;
							const char* appdate = nullptr;
							Appointment** allapps7 = appointments.getall();
							int acount7 = appointments.getsize();

							for (int a = 0; a < acount7; a++)
							{
								if (allapps7[a]->getid() == appid3 &&
									allapps7[a]->getdocid() == Doctorcurrent->getId() &&
									Appointment::Comparechars(allapps7[a]->getstatus(), "completed"))
								{
									appvalid = true;
									appdate = allapps7[a]->getdate();

									// check prescription not already written
									...
									int patid4 = allapps7[a]->getpatid();
									...
									// build prescription CSV line
									char presline[1000];
									int pli2 = 0;

									auto intToStr4 = [](int v, char* out) {
										if (v == 0) { out[0] = '0'; out[1] = '\0'; return; }
										char tmp[20]; int ti = 0;
										while (v > 0) { tmp[ti++] = '0' + v % 10;v /= 10; }
										int oi = 0;
									...
									char pp1[10]; intToStr4(newpresid, pp1);
									for (int x = 0;pp1[x] != '\0';x++) presline[pli2++] = pp1[x];
									presline[pli2++] = ',';

									char pp2[10]; intToStr4(appid3, pp2);
									for (int x = 0;pp2[x] != '\0';x++) presline[pli2++] = pp2[x];
									presline[pli2++] = ',';

									char pp3[10]; intToStr4(patid4, pp3);
									for (int x = 0;pp3[x] != '\0';x++) presline[pli2++] = pp3[x];
									presline[pli2++] = ',';

									char pp4[10]; intToStr4(Doctorcurrent->getId(), pp4);
									for (int x = 0;pp4[x] != '\0';x++) presline[pli2++] = pp4[x];
									presline[pli2++] = ',';

									for (int x = 0;appdate[x] != '\0';x++) presline[pli2++] = appdate[x];
									presline[pli2++] = ',';

									for (int x = 0;medicines[x] != '\0';x++) presline[pli2++] = medicines[x];
									presline[pli2++] = ',';

									for (int x = 0;notes[x] != '\0';x++) presline[pli2++] = notes[x];
									presline[pli2] = '\0';

									FileHandler::appendrecord("prescriptions.txt", presline);
					...
					else if (State == 25)
					{
						...
						else if (historybutton25.getGlobalBounds().contains(mousePosition))
						{
							...
							else
							{
								...
								char histstr[1000];
								int hli = 0;
								Prescription** allpres3 = prescriptions.getall();
								int pcount3 = prescriptions.getsize();

								for (int p = 0; p < pcount3; p++)
								{
									if (allpres3[p]->getpatientid() == patid5 &&
										allpres3[p]->getdoctorid() == Doctorcurrent->getId())
									{
										const char* dt9 = allpres3[p]->getdate();
										for (int x = 0;dt9[x] != '\0';x++) histstr[hli++] = dt9[x];
										histstr[hli++] = ' ';

										const char* med2 = allpres3[p]->getmedicines();
										for (int x = 0;med2[x] != '\0';x++) histstr[hli++] = med2[x];
										histstr[hli++] = '\n';

										const char* nt2 = allpres3[p]->getnotes();
										for (int x = 0;nt2[x] != '\0';x++) histstr[hli++] = nt2[x];
										histstr[hli++] = '\n';
										histstr[hli++] = '\n';
									}
								}
							...
							else
							{
								...
								else
								{
									...
									else
									{
										...
										adminresulttext371.setFillColor(sf::Color::Green);
										adminresulttext371.setString("Patient discharged and archived successfully.");
										dischargebox37.cleartext();
									}
								}
							}
		...
		else if (State == 11)
		{
			...
			// show doctor list based on specialization typed
			char docliststr[500];
			int dli = 0;
			...
			if (spec2[0] != '\0')
			{
				for (int d = 0; d < dcount3; d++)
				{
					...
					if (spec2[si2] != '\0' || ds2[si2] != '\0') match2 = false;

					if (match2)
					{
						const char* dn = alldocs3[d]->getName();
						for (int x = 0; dn[x] != '\0'; x++) docliststr[dli++] = dn[x];
						docliststr[dli++] = ' ';
						docliststr[dli++] = '(';
						// add id
						int dv2 = alldocs3[d]->getId();
						char dtmp2[10]; int dti2 = 0; char dstr2[10]; int dsi2 = 0;
						if (dv2 == 0) { dstr2[dsi2++] = '0'; }
						else { while (dv2 > 0) { dtmp2[dti2++] = '0' + dv2 % 10; dv2 /= 10; } for (int x = dti2 - 1; x >= 0; x--) dstr2[dsi2++] = dtmp2[x]; }
						dstr2[dsi2] = '\0';
						for (int x = 0; dstr2[x] != '\0'; x++) docliststr[dli++] = dstr2[x];
						docliststr[dli++] = ')';
						docliststr[dli++] = '\n';
					}
				}
		...
		else if (State == 12)
		{
			...
			for (int a = 0; a < acount3; a++)
			{
					...
					Appointment::Comparechars(allapps3[a]->getstatus(), "pending"))
				{
					...
					char atmp3[10]; int ati8 = 0; char astr4[10]; int asi8 = 0;
					if (av2 == 0) { astr4[asi8++] = '0'; }
					else { while (av2 > 0) { atmp3[ati8++] = '0' + av2 % 10; av2 /= 10; } for (int x = ati8 - 1; x >= 0; x--) astr4[asi8++] = atmp3[x]; }
					astr4[asi8] = '\0';
					const char* idlabel = "ID:";
					for (int x = 0; idlabel[x] != '\0'; x++) pendingstr[pli++] = idlabel[x];
					for (int x = 0; astr4[x] != '\0'; x++) pendingstr[pli++] = astr4[x];
					pendingstr[pli++] = ' ';

					// add date
					const char* dt3 = allapps3[a]->getdate();
					for (int x = 0; dt3[x] != '\0'; x++) pendingstr[pli++] = dt3[x];
					pendingstr[pli++] = ' ';

					// add slot
					const char* sl2 = allapps3[a]->getslot();
					for (int x = 0; sl2[x] != '\0'; x++) pendingstr[pli++] = sl2[x];
					pendingstr[pli++] = '\n';
				}
			}
			pendingstr[pli] = '\0';
		...
		else if (State == 16)
		{
			...
			for (int b = 0; b < bcount5; b++)
			{
					...
					Appointment::Comparechars(allbills5[b]->getstatus(), "unpaid"))
				{
					...
					char bstr5[10]; int bsi7 = 0; char btmp5[10]; int bti7 = 0;
					if (bv2 == 0) { bstr5[bsi7++] = '0'; }
					else { while (bv2 > 0) { btmp5[bti7++] = '0' + bv2 % 10; bv2 /= 10; } for (int x = bti7 - 1; x >= 0; x--) bstr5[bsi7++] = btmp5[x]; }
					bstr5[bsi7] = '\0';
					const char* bl2 = "BillID:";
					for (int x = 0; bl2[x] != '\0'; x++) unpaidstr[uli++] = bl2[x];
					for (int x = 0; bstr5[x] != '\0'; x++) unpaidstr[uli++] = bstr5[x];
					unpaidstr[uli++] = ' ';
					float amtv3 = allbills5[b]->getamount();
					int amtint3 = (int)amtv3;
					char amttmp3[10]; int amtti3 = 0; char amtstr3[20]; int amtsi3 = 0;
					if (amtint3 == 0) { amtstr3[amtsi3++] = '0'; }
					else { while (amtint3 > 0) { amttmp3[amtti3++] = '0' + amtint3 % 10; amtint3 /= 10; } for (int x = amtti3 - 1; x >= 0; x--) amtstr3[amtsi3++] = amttmp3[x]; }
					amtstr3[amtsi3] = '\0';
					const char* pkl = " PKR:";
					for (int x = 0; pkl[x] != '\0'; x++) unpaidstr[uli++] = pkl[x];
					for (int x = 0; amtstr3[x] != '\0'; x++) unpaidstr[uli++] = amtstr3[x];
					unpaidstr[uli++] = '\n';
				}
			}
			unpaidstr[uli] = '\0';
		...
		else if (State == 22)
		{
			...
			for (int a = 0; a < acount10; a++)
			{
					...
					Appointment::Comparechars(allapps10[a]->getdate(), todaystr5))
				{
					...
					char astr7[10]; int asi11 = 0; char atmp6[10]; int ati11 = 0;
					if (av5 == 0) { astr7[asi11++] = '0'; }
					else {
						while (av5 > 0) { atmp6[ati11++] = '0' + av5 % 10;av5 /= 10; }
						for (int x = ati11 - 1;x >= 0;x--) astr7[asi11++] = atmp6[x];
					}
					astr7[asi11] = '\0';
					const char* idl3 = "ID:";
					for (int x = 0;idl3[x] != '\0';x++) pendstr2[pli3++] = idl3[x];
					for (int x = 0;astr7[x] != '\0';x++) pendstr2[pli3++] = astr7[x];
					pendstr2[pli3++] = ' ';
					const char* sl7 = allapps10[a]->getslot();
					for (int x = 0;sl7[x] != '\0';x++) pendstr2[pli3++] = sl7[x];
					pendstr2[pli3++] = '\n';
				}
			}
			pendstr2[pli3] = '\0';
		...
		else if (State == 23)
		{
			...
			for (int a = 0; a < acount11; a++)
			{
					...
					Appointment::Comparechars(allapps11[a]->getdate(), todaystr6))
				{
					...
					char astr8[10]; int asi12 = 0; char atmp7[10]; int ati12 = 0;
					if (av6 == 0) { astr8[asi12++] = '0'; }
					else {
						while (av6 > 0) { atmp7[ati12++] = '0' + av6 % 10;av6 /= 10; }
						for (int x = ati12 - 1;x >= 0;x--) astr8[asi12++] = atmp7[x];
					}
					astr8[asi12] = '\0';
					for (int x = 0;astr8[x] != '\0';x++) pendstr3[pli4++] = astr8[x];
					pendstr3[pli4++] = ' ';
					const char* sl8 = allapps11[a]->getslot();
					for (int x = 0;sl8[x] != '\0';x++) pendstr3[pli4++] = sl8[x];
					pendstr3[pli4++] = '\n';
				}
			}
			pendstr3[pli4] = '\0';
